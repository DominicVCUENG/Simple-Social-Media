...
async def home(request: Request) -> HTMLResponse:
    ...
    return templates.TemplateResponse(request = request, name = "index.html", context = posts.model_dump())

@app.get("/posts")
async def posts(request: Request) -> HTMLResponse:
    posts = get_post(connection)
    return templates.TemplateResponse(request = request, name = "posts.html", context = posts.model_dump())
